...
class backtranslation_rewrite(Strategy):
    ...
    async def process_single_file(self, config, file_path: str, concurrent_api_requests_num: int):
        ...
        for i in range(0, len(datas), concurrent_api_requests_num):
            ...
            for data in batch_datas:
                ...
                for text in texts:
                    if(text == ""):
                        continue
                    task = self.process_single_data(config, text, file_path, main_theme, concurrent_api_requests_num)
                    tasks.append(task)
    ...
    2.每条关键信息必须包括{main_theme}相关字样。"""
            ...
            for title, extractions_text in zip(batch_titles, batch_extractions):
                ...
                extractions = clean_and_split_reply(extractions_text)
