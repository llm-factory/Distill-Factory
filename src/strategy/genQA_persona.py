...
class genQA_persona(Strategy):
    ...
    async def process_single_file(self, file_path: str, main_theme: str, num_question_per_title: int, concurrent_api_requests_num: int):
        ...
        questions = await self.generateQuestions(
            text, 
            main_theme, 
            num_question_per_title, 
            titles,
            personas,
            concurrent_api_requests_num=concurrent_api_requests_num
        )
        questions = questions_filter(questions)
        
        logger.info(f"{'=' * 30}Generating Answers For {file_path}{'='*30}")
        answers = await self.getAnswers(
            text, 
            questions, 
            concurrent_api_requests_num=concurrent_api_requests_num
        )
    ...
    async def generateQuestions(self, text, main_theme, num_question_per_title, titles, personas, concurrent_api_requests_num=1):
        questions = []
        ...
        for idx in tqdm(range(0, len(titles), concurrent_api_requests_num), desc='Generating questions'):
            ...
            for i in range(len(batch_titles)):
                ...
                logger.info(f"{'-'*20}Persona{'-'*20}")
                logger.info(persona)
                prompt = buildMessages([
                    UserMessage(
                        f"根据以下文本：\n{text}\n以'{persona}'的口吻指向{main_theme}生成{num_question_per_title}个在不同场景下与{batch_titles[i]}有关的可以根据文本内容回答的问题，问题必须明确指向{main_theme}以避免指向模糊。每个问题一行，以数字加.开始，不能重复"
                        f"""\n请以JSON格式输出，示例格式如下：\n" 
                            [
                                {{
                                    "Question": "问题1"
            ...
            for i in range(len(batch_titles)):
                ...
                prompt = buildMessages([
                    UserMessage(
                        f"根据以下文本：\n{text}\n以'{persona}'的口吻指向{main_theme}生成{num_question_per_title}个在不同场景下不含{batch_titles[i]}但需要结合文本中关于{batch_titles[i]}的知识才能回答的问题，每个问题一行，以数字加.开始，不能重复"
                        f"""\n请以JSON格式输出，示例格式如下：\n" 
                            [
                                {{
                                    "Question": "问题1"
                ...
                prompts.append(prompt)
            genQuestions = await self.api.async_chat(prompts)
            genQuestions = parseJsons(genQuestions)
            questions.extend(genQuestions)
        return questions
